...
cleanup() {
  docker rm -f "$container" "$proxy" "$preview" >/dev/null 2>&1 || true
...
MSYS_NO_PATHCONV=1 docker run --rm --entrypoint caddy "$image" validate --config /etc/caddy/Caddyfile.local --adapter caddyfile
docker network create "$network" >/dev/null
# Exercise the production default, including container-name resolution at the gateway.
docker run -d --name "$container" --network "$network" --network-alias xsolutions-site -p 127.0.0.1::80 "$image"
...
docker run -d --name "$proxy" --network "$network" -p 127.0.0.1::80 "$image" \
...
MSYS_NO_PATHCONV=1 docker run -d --name "$preview" -p 127.0.0.1::80 "$image" caddy run --config /etc/caddy/Caddyfile.local --adapter caddyfile
